...
WINDOW_FOLDER = os.path.join(config.OUTPUT_DIR, "window_data_wideband")
...
def get_session_range_from_cli():
    ...
    python run_cross_session_csp_5band.py 1 3
    """
    if len(sys.argv) == 1:
        return None, None
    ...
    raise ValueError("Kullanim: python run_cross_session_csp_5band.py [start_session end_session]")
...
def get_session_window_files():
    """
    outputs/window_data_wideband klasoru icindeki session window dosyalarini bulur.

    Beklenen dosya adi:
    session_1_wideband_windows.npz
    """
    ...
    for filename in os.listdir(WINDOW_FOLDER):
        if filename.endswith("_wideband_windows.npz") and filename.startswith("session_"):
            match = re.search(r"session_(\d+)_wideband_windows\.npz", filename)
    ...
    if len(session_files) == 0:
        raise ValueError("Hic wideband window dosyasi bulunamadi.")
...
def load_one_session_window(file_path):
    """
    Tek bir session'in wideband window dosyasini okur.
...
def load_all_sessions(start_session=None, end_session=None):
    """
    Tum mevcut session wideband window dosyalarini yukler.
    """
    ...
    if start_session is not None and end_session is not None:
        ...
        if len(sorted_ids) == 0:
            raise ValueError("Verilen aralikta wideband window dosyasi bulunamadi.")
...
def save_fold_results(fold_results):
    ...
    save_path = os.path.join(RESULTS_FOLDER, "loso_csp_5band_lda_results.csv")
...
def save_predictions(all_predictions):
    ...
    os.makedirs(RESULTS_FOLDER, exist_ok=True)
    save_path = os.path.join(RESULTS_FOLDER, "loso_csp_5band_lda_predictions.csv")
...
def main():
    ...
    print("Session wideband window dosyalari araniyor...")
    ...
    fold_results, all_predictions = run_loso_csp_5band_baseline(session_data, verbose=True)

    print_final_summary(fold_results)
    save_fold_results(fold_results)
    save_predictions(all_predictions)

    print("\nCross-session 5-band CSP kosusu tamamlandi.")
